  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <button onClick={() => navigate('/hives')} className="inline-flex items-center gap-1 text-sm text-stone-500 hover:text-stone-700">
      ...
      <div className="flex flex-col sm:flex-row items-start gap-4">
        {hive.photo ? (
          ...
          <div className="h-24 w-24 rounded-2xl bg-honey-100 flex flex-col items-center justify-center text-honey-500">
            <Hexagon className="h-10 w-10" />
          </div>
        )}
        <div className="flex-1">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-2xl font-bold text-stone-800">{t('dashboard.hiveNumber')}{hive.number}</h1>
            {hive.name && <span className="text-stone-500">— {hive.name}</span>}
      ...
      <div className="flex gap-1 bg-stone-100 rounded-xl p-1 overflow-x-auto scrollbar-thin">
        {TABS.map((tabKey) => (
          <button
            key={tabKey}
            onClick={() => setTab(tabKey)}
            className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition ${
              tab === tabKey ? 'bg-white text-honey-700 shadow-sm' : 'text-stone-500 hover:text-stone-700'
            }`}
          >
      ...
      {tab === 'info' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="p-5">
            <h3 className="font-semibold text-stone-800 mb-4">{t('hives.detail')}</h3>
          ...
          <Card className="p-5">
            <h3 className="font-semibold text-stone-800 mb-4">👑 {t('hives.queenPresent')}</h3>
            <div className="grid grid-cols-2 gap-4">
          ...
          {hive.notes && (
            <Card className="p-5 md:col-span-2">
              <h3 className="font-semibold text-stone-800 mb-2">{t('hives.notes')}</h3>
              <p className="text-sm text-stone-600">{hive.notes}</p>
            </Card>
          )}
        </div>
      )}
      ...
      {tab === 'history' && (
        <Card className="p-6">
          {history.length === 0 ? (
            ...
            <div className="relative ps-6 border-s-2 border-stone-100 space-y-6">
              {history.map((item, idx) => (
                <div key={idx} className="relative">
                  ...
                  {item.text && <p className="text-sm text-stone-600 mt-1">{item.text}</p>}
                  <div className="mt-2 flex gap-2">
                    <Link to={item.type === 'inspection' ? `/inspections/${item.id}` : `/tasks/${item.id}`} className="text-xs text-honey-600 hover:underline">
                      {t('common.view')}
                    </Link>
            ...
            <Link to={`/inspections/${insp.id}`} key={insp.id}>
              <Card className="p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-2">
                  <p className="font-semibold text-stone-800">{formatDate(insp.date)} {insp.time || ''}</p>
                  <Badge color={strengthColors[insp.strength]}>{t(`hiveStrength.${insp.strength}`)}</Badge>
                </div>
                <p className="text-sm text-stone-600">{insp.observations || '—'}</p>
              </Card>
            </Link>
          ))}
        </div>
        ...
        <div className="space-y-3">
          ...
          {tasks.length === 0 ? <EmptyState /> : tasks.map((task) => (
            <Link to={`/tasks/${task.id}`} key={task.id}>
              <Card className="p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between">
                  <p className="font-semibold text-stone-800">{t(`taskType.${task.type}`)}</p>
            ...
            <Card key={queen.id} className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-stone-500">{queen.origin || ''} • {queen.age ? `${queen.age} ${t('queens.age')}` : ''}</p>
                </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Card className="p-4 bg-red-50">
              <p className="text-sm text-red-700">{t('finances.totalExpenses')}</p>
              <p className="text-lg font-bold text-red-700">{formatMoney(totalExpenses, hive.currency)}</p>
            </Card>
            <Card className="p-4 bg-emerald-50">
              <p className="text-sm text-emerald-700">{t('finances.totalRevenues')}</p>
              <p className="text-lg font-bold text-emerald-700">{formatMoney(totalRevenues)}</p>
            </Card>
            <Card className="p-4 bg-honey-50">
              <p className="text-sm text-honey-700">{t('finances.netProfit')}</p>
              <p className="text-lg font-bold text-honey-700">{formatMoney(totalRevenues - totalExpenses)}</p>
            </Card>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card className="p-4">
              <h3 className="font-semibold text-stone-800 mb-3">🍯 {t('harvests.title')}</h3>
              {harvests.length === 0 ? <EmptyState /> : harvests.map((h) => (
                <div key={h.id} className="flex justify-between py-2 border-b border-stone-50 last:border-0">
                  <span>{formatDate(h.date)} — {h.honeyType || ''}</span>
                  <span className="font-medium">{h.quantity || h.weight || 0} kg</span>
                </div>
              ))}
            </Card>
            <Card className="p-4">
              <h3 className="font-semibold text-stone-800 mb-3">{t('finances.expenses')}</h3>
              {expenses.length === 0 ? <EmptyState /> : expenses.slice(0, 10).map((e) => (
                <div key={e.id} className="flex justify-between py-2 border-b border-stone-50 last:border-0">
                  <span>{formatDate(e.date)} — {t(`expenseCategory.${e.category}`)}</span>
